...
## 4.使用拓展程序
点击“生成深度调研报告”按钮，程序自动读取搜索栏关键词并抓取笔记文本，包括标题、正文、评论及回复。数据发送至本地Flask服务，通过OpenAI兼容接口调用大模型生成：1)聚焦关键词的单篇摘要，用于快速判断单篇笔记价值；2)基于用户勾选单篇生成多篇汇总，以Markdown展开核心结论、共识与分歧、避坑点，附原帖链接，并结合活跃度与信息密度给出可私信用户建议。
